...
/**
 * System prompt for the free-form roadmap chat. Unlike SYSTEM_INSTRUCTION there
 * is no graded problem or authored solution to anchor to, so the tutor may teach
 * and work examples freely — but it stays scoped to the course and uses the same
 * strict math/no-Markdown formatting rules so replies render identically.
 */
const GENERAL_SYSTEM_INSTRUCTION = `You are the Calculus Lab tutor, chatting with a high-school AP Calculus BC student who has opened a free-form chat from the course roadmap (not a graded problem).
...
function buildMessages(
  ...
  if (req.mode === "general") {
    const generalMessages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] =
      [{ role: "system", content: GENERAL_SYSTEM_INSTRUCTION }];
    if (req.general) {
      const summary = buildGeneralContextSummary(req.general);
      if (summary) generalMessages.push({ role: "user", content: summary });
    }
    ...
    generalMessages.push({ role: "user", content: req.message });
